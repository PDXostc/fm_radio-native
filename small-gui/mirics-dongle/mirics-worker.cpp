...
#include	"mirics-worker.h"	// our header
#include	"ringbuffer.h"		// the buffer

//	The mirics worker is the actual wrapper around the mir-sdr
...
#define	RATE_CHANGE	0004
	...
	miricsWorker::miricsWorker (int32_t	deviceRate,
	                            int32_t	bandWidth,
	                            int32_t	defaultFreq,
	                            miricsLoader	*f,
...
//	be done carefully.
void	miricsWorker::stop	(void) {
	runnable	= false;
}

	miricsWorker::~miricsWorker	(void) {
	runnable	= false;
	while (isRunning ())
	   msleep (1);
...
void	miricsWorker::setVFOFrequency	(int32_t f) {
	lastFrequency	= f;
	anyChange 	|= FREQ_CHANGE;
}

void	miricsWorker::setExternalGain	(int16_t gain) {
...
void	miricsWorker::setExternalRate	(int32_t newRate) {
